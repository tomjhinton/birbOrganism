...
void main()
{
  ...
  float t = (uTime * .5) + length(modelPosition.xy);

  float distance = length(uMousePosition - modelPosition.xyz);
  float squishFactor = smoothstep(0.0, 1., 3.0 - distance);
  coswarp(modelPosition.xyz, sin(t) * .5);
  modelPosition += (squishFactor * (sin(u_progress) ) ) * .2;
